...
public class Q11_20 {
    ...
    private static int problem11() {
        ...
        for(int i=0; i<17; i++) {
            for(int j=0; j<20; j++) {
                int horProduct=0, verProduct=0, diagProduct1=0, diagProduct2=0;

                if(j<17) {
                    horProduct = nums[i][j] * nums[i][j+1] * nums[i][j+2] * nums[i][j+3];
                    verProduct = nums[i][j] * nums[i+1][j] * nums[i+2][j] * nums[i+3][j];
                    diagProduct1 = nums[i][j] * nums[i + 1][j + 1] * nums[i + 2][j + 2] * nums[i + 3][j + 3];
                }
                if(j>3)
                    diagProduct2 = nums[i][j] * nums[i+1][j-1] * nums[i+2][j-2] * nums[i+3][j-3];

                product = Math.max(product, Math.max(
                        Math.max(horProduct, verProduct),
                        Math.max(diagProduct1, diagProduct2)));
            }
        }
    ...
    private static boolean divisors(long num, int k) {
        int factors = 0;
        for(long i=1; i<Math.sqrt(num); i++) {
            if(num%i==0) factors+=2;
            if(factors >= k) return true;
        }
        return false;
    }
